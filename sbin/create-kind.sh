...
# Copied and tuned from https://kind.sigs.k8s.io/docs/user/local-registry

set -o errexit

# Delete previous Kind cluster.
echo "Do you want to remove any existing crossplane-examples cluster (y/N)?"
read REPLY

if [[ $REPLY =~ ^[Yy]$ ]]
then
  kind delete cluster --name crossplane-examples
fi
...
cat <<EOF | kind create cluster --name crossplane-examples --config=-
kind: Cluster
